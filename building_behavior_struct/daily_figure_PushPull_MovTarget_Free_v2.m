...
elseif strcmp(person,'srinidhi')
    raw_folder ='C:\Users\srini\Learning Lab Dropbox\Learning Lab Team Folder\Patlab protocols\data\TD\behavior_data\raw_data';
end
...
%     sess_name(ss,1)=cellstr(convertCharsToStrings(sess(ss,1).name));
%     %sess_name_tmp(ss,1)=cellstr(convertCharsToStrings(sessionss{8,1}.session(ss,1))); %G2
% end
% PP_sess_tmp = sess_name(contains(sess_name,'PP'));
% PP_sess = PP_sess_tmp(2:end);
%
% for s = 16:length(PP_sess)
...
plot(ind_newTrial_event,ones(size(ind_newTrial_event)).*max(syncCam),'*'); hold off
...
figure
plot(time_newTrial_log,time_newTrial_cam,'-'); hold on
...
if sum(water_moved_all_ind<ind_first_trial_available)>1 
...
time_to_init = timelog(trial_init_ind)-timelog(trial_available_to_start_ind);
time_to_reach = timelog(reached_ind)-timelog(water_delivered_ind);
...
hold on
plot(trials_vec,time_to_reach,'k.');
